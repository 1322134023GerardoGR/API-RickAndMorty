...
<main class="align-items-center">
    ...
    try {
        ...
        if ($specie != "") {
            if ($specie == "MythologicalCreature") {
                $specie= "Mythological Creature";
            }
                $characters = array_filter($characters, function ($character) use ($specie) {
                    return $character['species'] == $specie;
                });
        }else{
            echo '<h2>Personajes aleatorios</h2>';
